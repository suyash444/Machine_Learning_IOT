...
class MFCC():
    ...
    def _compute_log_mel_spectrogram(self, audio):
        return self.mel_spectrogram_processor.get_mel_spec(audio)

    def _compute_mfccs(self, log_mel_spectrogram):
        mfccs = tf.signal.mfccs_from_log_mel_spectrograms(log_mel_spectrogram)[..., :self.num_coefficients]
        return mfccs

    def get_mfccs(self, audio):
        log_mel_spectrogram = self._compute_log_mel_spectrogram(audio)
        return self._compute_mfccs(log_mel_spectrogram)
